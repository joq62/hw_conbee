...
-define(Types,<<"ZHAPressure">>,<<"ZHAHumidity">>,<<"ZHATemperature">>).
...
humidity({[],Name,WantedNumDeviceMaps})->
    gen_server:call(?SERVER, {state_get,<<"humidity">>,{[],Name,WantedNumDeviceMaps}},infinity). 
temperature({[],Name,WantedNumDeviceMaps})->
    gen_server:call(?SERVER, {state_get,<<"temperature">>,{[],Name,WantedNumDeviceMaps}},infinity). 
...
handle_call({num,{[],Name,NumMaps}},_From, State) ->
    [{NumPressure,_MapPressure}]=lib_hw_conbee:get_nummap(Name,<<"ZHAPressure">>,NumMaps),
    [{NumHumidity,_MapHumidity}]=lib_hw_conbee:get_nummap(Name,<<"ZHAHumidity">>,NumMaps),
    [{NumTemperature,_MapTemperature}]=lib_hw_conbee:get_nummap(Name,<<"ZHATemperature">>,NumMaps),
    Reply=[{pressure,binary_to_list(NumPressure)},
	   {humidity,binary_to_list(NumHumidity)},
	   {pressure,binary_to_list(NumTemperature)}],
    {reply, Reply, State};


handle_call({basic,Key,{[],Name,NumMaps}},_From, State) ->
    [{_NumPressure,MapPressure}]=lib_hw_conbee:get_nummap(Name,<<"ZHAPressure">>,NumMaps),
    Reply=binary_to_list(maps:get(Key,MapPressure)),
    {reply, Reply, State};

%% state

handle_call({state_get,<<"humidity">>,{[],Name,NumMaps}},_From, State) ->
    [{_NumHumidity,MapHumidity}]=lib_hw_conbee:get_nummap(Name,<<"ZHAHumidity">>,NumMaps),
    DeviceMap=maps:get(<<"state">>,MapHumidity),
    Reply=maps:get(<<"humidity">>,DeviceMap),
    {reply, Reply, State};

handle_call({state_get,<<"temperature">>,{[],Name,NumMaps}},_From, State) ->
    [{_NumTemperature,MapTemperature}]=lib_hw_conbee:get_nummap(Name,<<"ZHATemperature">>,NumMaps),
    DeviceMap=maps:get(<<"state">>,MapTemperature),
    Reply=maps:get(<<"temperature">>,DeviceMap),
    {reply, Reply, State};

handle_call({state_get,<<"pressure">>,{[],Name,NumMaps}},_From, State) ->
    [{_NumPressure,MapPressure}]=lib_hw_conbee:get_nummap(Name,<<"ZHAPressure">>,NumMaps),
    DeviceMap=maps:get(<<"state">>,MapPressure),
...
handle_call({config_get,Key,{[],Name,NumMaps}},_From, State) ->
    [{_NumPressure,MapPressure}]=lib_hw_conbee:get_nummap(Name,<<"ZHAPressure">>,NumMaps),
    ConfigMap=maps:get(<<"config">>,MapPressure),
